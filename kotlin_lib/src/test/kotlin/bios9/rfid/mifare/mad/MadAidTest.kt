...
class MadAidTest {
  ...
  fun `create from raw methods should be consistent`() {
    for (functionClusterCode in UByte.MIN_VALUE..UByte.MAX_VALUE) for (applicationCode in
  ...
  fun `toString no admin code`() {
    for (function in 1u..255u) {
      ...
      for (app in 0u..255u) {
        ...
        assertEquals(
            "${function} - ${app} (0x${aid.rawValue.toHexString(HexFormat.UpperCase)})",
            aid.toString())
      }
    }
  }
  ...
  fun `toString admin code`() {
    ...
    for (app in 0u..255u) {
      ...
      val aid = MadAid.fromFunction(function, app.toUByte())
      assertEquals(
          "${adminCode} (0x${aid.rawValue.toHexString(HexFormat.UpperCase)})", aid.toString())
    }
  }
