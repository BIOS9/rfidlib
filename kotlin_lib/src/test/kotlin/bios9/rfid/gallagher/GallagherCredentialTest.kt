...
class GallagherCredentialTest {
  ...
  fun `valid credential creation should succeed`() {
    val credential =
        GallagherCredential.create(
            regionCode = 3u, facilityCode = 100u, cardNumber = 500000u, issueLevel = 2u)
  ...
  fun `region code letter should be calculated correctly`() {
    val testCases = listOf(0 to 'A', 1 to 'B', 5 to 'F', 10 to 'K', 15 to 'P')
  ...
  fun `invalid region code should throw exception`() {
    val exception =
        assertThrows<IllegalArgumentException> {
          GallagherCredential.create(
              regionCode = 16u, facilityCode = 100u, cardNumber = 500000u, issueLevel = 2u)
        }
  ...
  fun `invalid card number should throw exception`() {
    val exception =
        assertThrows<IllegalArgumentException> {
          GallagherCredential.create(
              regionCode = 2u, facilityCode = 50u, cardNumber = 0x1000000u, issueLevel = 1u)
        }
  ...
  fun `invalid issue level should throw exception`() {
    val exception =
        assertThrows<IllegalArgumentException> {
          GallagherCredential.create(
              regionCode = 1u, facilityCode = 45u, cardNumber = 123456u, issueLevel = 16u)
        }
  ...
  fun `toString should correctly format output`() {
    val credential =
        GallagherCredential.create(
            regionCode = 3u, facilityCode = 45u, cardNumber = 123456u, issueLevel = 2u)
    assertEquals(
  ...
  fun `encoding and then decoding should return original credential`() {
    for (rc in 0u..0x0Fu step 2) for (fc in 0u..0xFFFFu step 500) for (cn in
        0u..0xFFFFFFu step 2000) for (il in 0u..0x0Fu step 3) {
  ...
  fun `invalid data length should fail`() {
    assertThrows<IllegalArgumentException> { GallagherCredential.decode(UByteArray(0)) }
    assertThrows<IllegalArgumentException> { GallagherCredential.decode(UByteArray(7)) }
    assertThrows<IllegalArgumentException> { GallagherCredential.decode(UByteArray(9)) }
  }
}
