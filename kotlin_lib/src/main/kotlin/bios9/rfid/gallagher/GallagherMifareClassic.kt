...
class GallagherMifareClassic private constructor(val credentials: List<GallagherCredential>) {
  companion object {
    const val GALLAGHER_CAD_AID =
        0x4811u // MAD Application ID of Gallagher Card Application Directory.
    const val GALLAGHER_CREDENTIAL_AID = 0x4812u // MAD Application ID of Gallagher Credential.

    //        /**
    //         * @param mifareApplicationDirectory Existing MAD data, null if none exists. This must
    // be
    // provided if overwriting existing MAD applications is not intended.
    //         */
    //        fun create(mifareApplicationDirectory: MifareApplicationDirectory?, something map of
    // creds
    ...
    //                .addCredential(myCred1, sector15, keya, keyb, accessRights?) // Custom sector
    // and
    // keys
    ...
    fun readFromTag(
      ...
      val cadSector =
          mad.applications.entries
              .firstOrNull { (_, aid) -> aid.rawValue == GALLAGHER_CAD_AID.toUShort() }
              ?.key
      val credentialSectors =
          if (cadSector != null) {
            ...
            Logger.d {
              "Card Application Directory (CAD) not found. Searching for credentials in MAD..."
            }
            val credentialSectors =
                mad.applications.entries
                    ...
                    .toSet()
            Logger.d {
              "Found ${credentialSectors.size} Gallagher credentials in MAD. Sectors: $credentialSectors"
            }
      ...
      val credentials =
          credentialSectors.mapNotNull { sector ->
    ...
    private fun readCredentialSector(
        tag: MifareClassic,
        sector: Int,
        keyProvider: MifareClassicKeyProvider
    ): GallagherCredential {
      ...
      val mes =
          tag.readBlock(
              MifareClassic.sectorToBlock(sector, 3)) // Gallagher Mifare Enhanced Security (MES)
  ...
  //    // or do I want a class for each credential type. <- probably this one in some way, it's
  // gonna
  ...
  //    // This class needs to store a set of credentials, which sectors those credentials were in
  // so
  // I can regenerate the MAD the same way.
  //    // It should also not overwrite other stuff on the card. Like if you read a tag with
  // gallagher
  ...
  //        // Kind of nice to be able to write to multiple different tags with different keys
  // without
