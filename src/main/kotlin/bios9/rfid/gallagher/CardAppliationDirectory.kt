...
class CardAppliationDirectory private constructor(val credentials: Map<Pair<UByte, UShort>, Int>) {
  companion object {
    val CAD_KEY_A: UByteArray =
        ubyteArrayOf(0xA0u, 0xA1u, 0xA2u, 0xA3u, 0xA4u, 0xA5u) // Same as MAD key.
    val CAD_KEY_B: UByteArray =
        ubyteArrayOf(0xB0u, 0xB1u, 0xB2u, 0xB3u, 0xB4u, 0xB5u) // Same as MAD key.
    val CAD_ACCESS_BITS: UByteArray =
    ...
    fun readFromMifareClassic(
      ...
      val expectedCrc =
          ((sectorData[0].toUInt() shl 8) or sectorData[1].toUInt())
              .toUShort() // Convert 2 CRC bytes into a short
      ...
      // They're an odd size of 3.5 bytes which means it's not very clean to read them byte by byte
      // so we have to use something to handle that.
      ...
      val mappings =
          (0..11)
              ...
              .takeWhile { (_, value) ->
                value != 0
              } // Stop when sector 0 is reached. Sector 0 cannot be used.
              .toMap()
  ...
  fun writeToMifareClassic(
      tag: MifareClassic,
      sector: Int,
      keyProvider: MifareClassicKeyProvider,
      cad: CardAppliationDirectory
  ) {
    require(sector in 1..39) { "Card Application Directory must be in sector 1 to 39" }
    keyProvider.authenticate(tag, sector)
