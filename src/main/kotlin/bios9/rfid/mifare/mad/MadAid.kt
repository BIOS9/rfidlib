 ...
 *
 * Registered function cluster codes can be found in
 * https://www.nxp.com/docs/en/application-note/AN10787.pdf
 */
class MadAid private constructor(val rawValue: UShort) {
  companion object {
    ...
    fun fromRaw(functionClusterCode: UByte, applicationCode: UByte): MadAid {
      return fromRaw(((functionClusterCode.toUInt() shl 8) or applicationCode.toUInt()).toUShort())
    }

    fun fromAdministrationCode(administrationCode: MadAdministrationCode): MadAid {
      return fromRaw(administrationCode.applicationCode!!.toUShort())
    }

    fun fromFunction(functionCluster: MadFunctionCluster, applicationCode: UByte): MadAid {
      requireNotNull(functionCluster.functionClusterCode) {
        "Cannot create AID using reserved function cluster."
      }
      return MadAid(
          ((functionCluster.functionClusterCode.toUInt() shl 8) or applicationCode.toUInt())
              .toUShort(),
      )
    }
  }
  ...
  // If the function cluster is zero, the application code is treated as an administration code
  // according to the spec.
  val administrationCode: MadAdministrationCode? =
      if (functionClusterCode == 0u.toUByte())
          MadAdministrationCode.fromApplicationCode(applicationCode)
      else null
