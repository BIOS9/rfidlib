...
impl CardApplicationDirectory {
    ...
    pub(crate) fn from_bytes(data: &[u8; 48], sector_hint: u8) -> Result<Self, Error> {
        let expected_crc = (u16::from(data[0]) << 8) | u16::from(data[1]);
        ...
        for raw in &raw_mappings {
            ...
            // Bit-field extraction: mask ensures only the target bits remain before truncation.
            let region_code = ((raw & 0x0F00_0000) >> 24) as u8;
            #[allow(clippy::cast_possible_truncation)]
            let facility_code = ((raw & 0x00FF_FF00) >> 8) as u16;
    ...
    pub(crate) fn to_bytes(&self) -> [u8; 48] {
        ...
        for (i, ((region_code, facility_code), cred_sector)) in self.mappings.iter().enumerate() {
            raw_mappings[i] = (u32::from(*region_code) << 24)
                | (u32::from(*facility_code) << 8)
                | u32::from(*cred_sector);
        }
        ...
        #[allow(clippy::cast_possible_truncation)]
        // Splitting a u16 into two u8 bytes — intentional.
        {
...
fn crc16(data: &[u8]) -> u16 {
    ...
    #[allow(clippy::cast_possible_truncation)]
    // Mask ensures only the low 16 bits remain before truncation.
    {
        (crc & 0xFFFF) as u16
    }
}
...
mod tests {
    ...
    fn crc16_known_vectors() {
        ...
        let v = [
            0x00u8, 0x01, 0xC1, 0x33, 0x70, 0xFD, 0x13, 0x38, 0x0D, 0x00, 0x00, 0x00, 0x00, 0x00,
        ...
        assert_eq!(
            crc16(&[
                0x00, 0x01, 0x00, 0x0D, 0xE0, 0xF0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00, 0x00, 0x00, 0x00
            ]),
        ...
        assert_eq!(
            crc16(&[
                0xBA, 0x7B, 0xA7, 0x93, 0x3C, 0x8C, 0x02, 0x76, 0xC0, 0x1C, 0xAF, 0x92, 0x44, 0xCB,
                0x96, 0x59, 0x5E, 0x6B, 0xF3, 0xA1, 0x63, 0xEB, 0xFB, 0x17, 0x82, 0x96, 0xD8, 0xBE,
                0xAE, 0xD9, 0xFF, 0x15, 0x5D, 0xD0, 0x43, 0xA9, 0xF3, 0xEC, 0x0B, 0x76, 0xE9, 0xC5,
                0x40, 0x2A, 0x0C, 0x8D
            ]),
            0x199E
        );
        assert_eq!(
            crc16(&[
                0x99, 0x08, 0x8C, 0x9E, 0xEA, 0xAE, 0x70, 0xB7, 0x19, 0xCE, 0x69, 0x16, 0x23, 0x1D,
                0x53, 0x3D, 0xB6, 0x04, 0xEC, 0xF3, 0x02, 0x8A, 0xFE, 0x97, 0xB9, 0x21, 0x23, 0xA1,
                0xD1, 0x2E, 0x5F, 0x98, 0x35, 0xB9, 0xDF, 0x70, 0xEB, 0x01, 0x79, 0x5C, 0xE6, 0xDD,
                0x44, 0x5A, 0x47, 0x97
            ]),
            0x0976
        );
        assert_eq!(
            crc16(&[
                0xA1, 0xD5, 0xC8, 0x06, 0xCE, 0xE8, 0xE1, 0x10, 0x36, 0xFD, 0x54, 0x9A, 0x45, 0x6C,
                0xCC, 0x02, 0x9B, 0x15, 0xA6, 0xE6, 0x1D, 0x0F, 0xF6, 0x44, 0xEB, 0x59, 0x03, 0x3C,
                0x47, 0x57, 0x14, 0xA9, 0x7C, 0x88, 0x0A, 0x54, 0x4F, 0x96, 0x83, 0x78, 0x46, 0x5A,
                0xB7, 0xDC, 0x4C, 0xBF
            ]),
            0x1E40
        );
        assert_eq!(
            crc16(&[
                0x03, 0xB4, 0xED, 0xEA, 0x8B, 0x71, 0xFD, 0x44, 0x56, 0x40, 0xAD, 0xF4, 0xB6, 0xB4,
                0xF5, 0xDC, 0x0C, 0x04, 0xBD, 0x5D, 0x7B, 0x81, 0x0A, 0x0A, 0x3D, 0x73, 0xAF, 0xEF,
                0xD6, 0x60, 0xCF, 0xFA, 0x86, 0x17, 0xFE, 0x65, 0x80, 0x20, 0xF4, 0x58, 0x25, 0x7B,
                0xDD, 0x29, 0x53, 0x65
            ]),
            0x036E
        );
        assert_eq!(
            crc16(&[
                0xF8, 0xC5, 0x75, 0x36, 0x3A, 0x5E, 0x31, 0x84, 0xFE, 0x9B, 0x93, 0x39, 0x0D, 0x11,
                0x7B, 0x02, 0xC9, 0x09, 0xFE, 0x63, 0x58, 0x24, 0x07, 0x33, 0x83, 0xFE, 0x7E, 0x2D,
                0xCE, 0xD4, 0xC8, 0x50, 0x07, 0x44, 0x8E, 0xA6, 0xB1, 0x72, 0x05, 0x81, 0x7D, 0xE9,
                0xCA, 0x3F, 0xAF, 0xE9
            ]),
            0x026D
        );
        assert_eq!(
            crc16(&[
                0xD9, 0x5C, 0xCF, 0x5C, 0x58, 0x2F, 0x4D, 0x2D, 0xF7, 0xC9, 0xE3, 0x8E, 0x0E, 0xA0,
                0x3E, 0x3B, 0x3B, 0x9F, 0x8B, 0x9C, 0x55, 0x13, 0xC2, 0xF0, 0x76, 0xA1, 0x54, 0x96,
                0x53, 0x04, 0xF6, 0xF9, 0x78, 0xAF, 0x7A, 0x9E, 0xE9, 0x3D, 0x3F, 0xBF, 0x69, 0xA3,
                0x2B, 0x0C, 0x4B, 0x81
            ]),
            0x1B6C
        );
        assert_eq!(
            crc16(&[
                0xE9, 0xA3, 0xCD, 0x12, 0xED, 0x2F, 0x80, 0x0C, 0x2C, 0x76, 0x52, 0x0A, 0x34, 0x65,
                0xFC, 0x3F, 0xF7, 0xC3, 0x3E, 0xA5, 0xEB, 0xDE, 0x1B, 0x7D, 0xEE, 0x84, 0x8F, 0xC6,
                0x3E, 0x6D, 0xA6, 0x28, 0x3C, 0x92, 0x57, 0xF8, 0xFF, 0xA0, 0x82, 0x15, 0x11, 0x59,
                0x61, 0x6B, 0x2D, 0xD4
            ]),
            0x1DE0
        );
        assert_eq!(
            crc16(&[
                0x60, 0xE1, 0x02, 0x3B, 0xAC, 0x65, 0x5E, 0xAD, 0x01, 0x77, 0x24, 0xA8, 0x96, 0x9A,
                0xCD, 0x0D, 0xA2, 0x8A, 0x14, 0xFC, 0x4B, 0x23, 0x07, 0xE5, 0x4A, 0x16, 0xD0, 0x64,
                0x52, 0xFF, 0x10, 0x4E, 0x96, 0x70, 0x08, 0xC4, 0x6C, 0xF8, 0x84, 0xCD, 0xFC, 0x8C,
                0xB6, 0xB4, 0x10, 0xE1
            ]),
            0x04E2
        );
        assert_eq!(
            crc16(&[
                0x20, 0x08, 0x47, 0x8A, 0x53, 0x20, 0x49, 0x29, 0x50, 0xA9, 0x62, 0x57, 0x17, 0x16,
                0x67, 0x89, 0xC4, 0x37, 0xB0, 0x96, 0x45, 0x63, 0x42, 0xC8, 0xCD, 0xEB, 0xCE, 0x00,
                0x71, 0x26, 0xFA, 0xDD, 0xF2, 0x3F, 0xE4, 0xB3, 0x5C, 0xD6, 0x55, 0x71, 0x7C, 0x8A,
                0xEC, 0x06, 0x37, 0x90
            ]),
            0x0880
        );
        assert_eq!(
            crc16(&[
                0x9D, 0xB8, 0x41, 0x10, 0x53, 0x42, 0x4B, 0xAA, 0xDD, 0x3C, 0x67, 0x21, 0x22, 0xA1,
                0xD7, 0x1D, 0xFB, 0x17, 0x10, 0x2F, 0x47, 0x11, 0x04, 0x37, 0x3B, 0xDB, 0x41, 0x9D,
                0x71, 0xB9, 0xCA, 0x4D, 0xC3, 0x2F, 0xE0, 0x52, 0xC4, 0xF1, 0x2B, 0xBA, 0x5F, 0xE4,
                0xCF, 0xB6, 0x0A, 0xDB
            ]),
            0x064A
        );
    ...
    fn from_bytes_valid_real_world_sector() {
        ...
        let data: [u8; 48] = [
            0x1B, 0x58, 0x00, 0x01, 0xC1, 0x33, 0x70, 0xFD, 0x13, 0x38, 0x0D, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
