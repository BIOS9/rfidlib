...
impl MifareApplicationDirectory {
    ...
    pub fn read_from_tag<T: Tag>(
        ...
        for (i, chunk) in mad_v1_data[2..].chunks(2).enumerate() {
            ...
            if aid != MadAid::CardAdministration(AdministrationCode::Free) {
                let sector_index = u8::try_from(i).expect("MADv1 application index fits in u8");
                let sector = Sector::try_from(sector_index + 1)?; // +1 to skip sector 0.
                ...
                if applications.push((non_mad, aid)).is_err() {
        ...
        if mad_version == MadVersion::V2 {
            ...
            let expected_crc_v2 = mad_v2_data[0]; // First byte in sector 16 is the CRC. Skip it for CRC input too.
            if crc8(mad_v2_data.iter().skip(1)) != expected_crc_v2 {
                return Err(MadError::CrcMismatch);
            }
            ...
            for (i, chunk) in mad_v2_data[2..].chunks(2).enumerate() {
                ...
                // Ignore sectors with no application.
                if aid != MadAid::CardAdministration(AdministrationCode::Free) {
                    let sector_index = u8::try_from(i).expect("MADv2 application index fits in u8");
                    let sector = Sector::try_from(sector_index + 17)?; // +17 to skip MADv1 sectors and sector 16.
                    let non_mad = NonMadSector::try_from(sector)
                        .map_err(|_| MadError::InvalidApplicationSector(sector))?;
    ...
    pub fn write_to_tag<T: Tag>(
        ...
        let info_byte_v1 = if let Some(cps) = self.card_publisher_sector {
            ...
            let max_mad_v1_sector = FourBlockSector::S15 as u8;
            let cps_sector = u8::from(cps);

            // If CPS is not valid for MADv1, set it to 0.
            if cps_sector > max_mad_v1_sector {
                0u8
            } else {
                cps.into()
            }
        ...
        let gpb = 0b1000_0000 | // DA bit (MAD available) must be true since we're writing a MAD.
            if self.multi_application_card { 0b0100_0000 } else { 0b0000_0000 } | // MA bit (Multi Application Card)
            if self.mad_version == MadVersion::V1 { 0b0000_0001 } else { 0b0000_0010 }; //ADV bits (MAD version 1 or 2).
        ...
        if self.mad_version == MadVersion::V2 {
            ...
            // Write GPB at 10th byte of block 3.
            // AN10787 specifies "The GPB for MAD version 2 in sector 16 will be set to RFU (0x00)."
            data[57] = 0x00;
...
mod test {
    ...
    impl Tag for MockClassic4k<'_> {
        fn authenticate(
            ...
            assert!(
                key.eq(self.key) || key_type != KeyType::KeyA,
                "Unexpected key"
            );

            match sector {
                Sector::FourBlock(FourBlockSector::S0 | FourBlockSector::S16) => Ok(()),
    ...
    impl Tag for MockWritableClassic1k<'_> {
        fn authenticate(
            &mut self,
            sector: Sector,
            key: &[u8; 6],
    ...
    impl Tag for MockWritableClassic4k<'_> {
        fn authenticate(
            &mut self,
            sector: Sector,
            key: &[u8; 6],
            ...
            assert!(
                key.eq(self.key) || key_type != KeyType::KeyB,
                "Unexpected key"
            );
    ...
    impl KeyProvider for MockKeyProvider<'_> {
    ...
    const VALID_SECTOR_16: &[u8; 64] = b"";
    ...
    impl SectorHelpers for [u8; 64] {
        ...
        fn to_mad_v2(&self) -> [u8; 64] {
            assert_eq!(self[57], 0xC1, "Expected MADv1 sector");
    ...
    fn crc8_value() {
        ...
        assert_eq!(crc8(b"\x01\x01\x08\x01\x08\x01\x08\x00\x00\x00\x00\x00\x00\x04\x00\x03\x10\x03\x10\x02\x10\x02\x10\x00\x00\x00\x00\x00\x00\x11\x30".iter()), 0x89);
        assert_eq!(crc8(b"\x5D\x0A\xA9\xC6\x2A\xE2\xBD\x2D\xF1\xCD\xB2\x6C\xE1\x9D\xF6\x89\x38\xDA\x1D\x91\xC6\x76\x32\xCA\xC6\x48\x4A\xA4\x75\xB7\x46".iter()), 0x05);
        assert_eq!(crc8(b"\x3A\x83\x0D\xE7\xB0\xFF\x77\x66\xB3\xED\x0F\xE5\xD2\x55\x55\x34\x13\x8A\x7A\xB0\x5E\x5E\x6A\xBD\xE3\xFD\xF3\xBA\xA3\x05\x85".iter()), 0x3B);
        assert_eq!(crc8(b"\x03\x3F\xA2\xF6\xC0\x80\x41\x55\xAE\x74\x74\x45\x38\xD3\xDF\xCF\xEA\xE7\xEA\x9B\xCE\xAD\x5A\xEF\x7B\x07\x81\xE4\x1B\x09\x44".iter()), 0x64);
        assert_eq!(crc8(b"\xCD\x2C\xDC\x1C\xCC\xC1\xC5\xAB\x85\xA1\x99\x8B\xD4\x10\x00\x11\x9E\x03\xA1\x8A\xCC\x85\xC8\x8C\xE0\x00\xB7\x45\x17\x07\xF6".iter()), 0xE7);
        assert_eq!(crc8(b"\x3C\x36\xB2\x2C\x5C\xD3\xBC\x2D\x99\xBD\x8C\xFF\xB2\x2E\x30\xA0\xE2\xDF\x4E\x70\xCE\xBF\x8F\x82\x35\x43\x65\xCF\x13\x06\xC2".iter()), 0x78);
        assert_eq!(crc8(b"\xEB\xB8\x3C\x69\xE9\xCE\x8E\x40\x38\xEA\xFF\xAC\x11\xC4\xD9\x67\x2F\x12\xE3\x2E\x98\xBF\x67\xE4\xC5\x61\x1A\x5A\xAA\xA3\xBA".iter()), 0x13);
        assert_eq!(crc8(b"\x2A\xD8\xDE\x0B\x5C\xC3\x70\xB5\x0E\xD2\x6C\x3F\xD3\xC8\xD9\x5B\xFC\x83\x77\x09\xC3\x10\xF6\xB9\x23\xB9\x44\x73\xFA\x27\x55".iter()), 0xDE);
        assert_eq!(crc8(b"\x74\x3B\xD3\x86\x3F\x76\x3A\xBE\xE9\x6C\x6D\x80\x04\x88\xFB\x55\x73\xE2\x6D\x97\x21\xA1\xAE\xCB\xFD\x66\xDF\xCC\xBD\x0D\x07".iter()), 0x6D);
        assert_eq!(crc8(b"\x60\xEA\x81\x4D\x3E\x8F\x05\xFE\xF1\xAB\x52\x44\xD3\x30\xFA\x76\x8C\xF1\x3D\xCE\xD4\x50\x57\x10\xB1\x7D\x10\x55\x93\xE3\x79".iter()), 0x74);
        assert_eq!(crc8(b"\x8D\x2B\x76\xBF\x9D\x47\x8E\xC6\x91\x19\xE8\xAA\xED\xB3\x01\x89\xBB\x9D\xDA\xDA\x70\x3E\xF9\xE0\xE7\x51\xC0\x36\xF1\x44\x8A".iter()), 0x31);
        assert_eq!(crc8(b"\x54\x3D\x2B\x50\xC7\x0B\xF7\x0B\x2B\x80\x94\x5D\xBB\x07\x7E\xD3\xBB\xAF\xE1\x63\xBA\x98\xD6\x4D\x64\x5E\x51\x2C\x58\x08\x0E\x47\x3E\x52\xA5\x8D\x92\xB2\x43\x3B\x6D\x53\x02\x8C\x12\xD0\xC4".iter()), 0xC8);
        assert_eq!(crc8(b"\x77\xEE\xCF\x65\x1E\x46\x9C\xDB\x6B\xC3\x06\x16\xB4\xF7\x63\x1C\x6B\x07\xFD\xCA\x44\x19\x31\x19\x7E\x87\x94\x26\xF8\xD1\xDA\xAD\xD3\xA3\x1D\x5A\x5D\x99\xA7\xDA\xD0\xA4\x97\xA4\xBE\x34\x4F".iter()), 0x02);
        assert_eq!(crc8(b"\xB1\x4E\x34\x69\x6C\xA2\x5D\x83\xA5\xF6\xA6\x4B\xB1\x10\x7A\x1D\x11\xBE\x15\x91\x31\x3E\xFE\xD7\xA1\x88\xB5\x54\x0F\xF2\xC1\xAB\xFF\xD8\x6D\x75\xA1\xD2\xE8\x9C\x66\xE4\x9F\x0B\x35\x09\x29".iter()), 0x99);
        assert_eq!(crc8(b"\x66\xDB\x7A\x27\xC7\x06\x4A\xED\xE4\xE0\x48\xC4\x04\x38\xF5\x65\xBA\x5A\xB4\xFC\xD6\x54\xA0\xBC\xA5\xB8\x70\x7E\xE7\xF3\x3A\x38\x37\xFC\xD7\xE4\x3E\xE6\x9D\xF1\x48\x87\xE5\x8E\x81\x81\xCB".iter()), 0x6D);
        assert_eq!(crc8(b"\x58\x7A\xB6\xD5\xC0\x16\x2B\x29\x4B\xA4\x4D\xD4\x42\xB0\x89\xD4\xCA\xC8\x29\x91\xAF\xBD\x7D\xEE\xE6\xE3\x7D\xC0\x17\x7B\x09\x6F\x33\x6A\x47\x4C\x30\x26\xA9\x46\x34\x97\x12\x1C\x7D\x80\x85".iter()), 0xB1);
        assert_eq!(crc8(b"\x72\x93\xF4\xF1\xD9\x27\xA2\x50\x30\x2D\x7C\x98\x02\x05\xBE\x13\x50\xDA\x9B\x09\x12\xE3\xA6\x23\x29\xA5\xF4\x80\x70\x62\x7B\x61\x9F\x15\xDE\x5F\x9E\xCA\x36\xDE\x0C\xCA\xFA\x63\x13\x8A\xA2".iter()), 0x11);
        assert_eq!(crc8(b"\x9B\x41\x39\x95\xAA\xC0\x7C\x55\x71\x87\x48\xB8\xA0\x28\x7A\x12\x73\x07\x9A\x3A\xCB\xC0\x49\x78\x92\xE5\x24\x82\x7A\x57\x80\x94\x24\x5B\xE9\xC9\x28\xBF\x05\xAE\x76\xD7\xB2\x3F\xF2\x26\x14".iter()), 0xD0);
        assert_eq!(crc8(b"\x20\x1C\xF6\x9D\xA3\xEB\x4B\x85\x0C\xC1\xB4\x39\xC4\x64\x5B\x16\x61\x14\xDC\xEA\xF3\xB6\x9D\x40\x31\xE9\x3B\x22\x2C\xD5\x52\x52\x21\xA4\xDC\xE7\x16\x0C\x48\x30\x86\x2C\xA4\x92\x44\x92\x53".iter()), 0x87);
        assert_eq!(crc8(b"\x46\x87\xA7\xB7\x19\xA2\x76\xA6\x53\x1F\x8D\x8C\xDD\x67\x9B\x1B\xAC\x35\x0E\xAC\xB2\x82\x92\x25\x47\xAA\x68\x51\x09\xCA\xEB\xC5\x20\x8F\x2E\xC2\x97\xF7\x03\x72\xD9\xC6\x5B\x5B\x2F\x04\xBB".iter()), 0xE8);
        assert_eq!(crc8(b"\x6E\xF1\x9C\x0D\xCC\xF4\x73\x67\xBE\x62\xC4\xBA\x37\x4B\xAF\x0D\x8A\xE6\xA1\xA7\xC5\xC8\xB9\xC7\x87\xF3\x80\xEC\x42\x46\x5A\xB7\x06\x2A\x33\xC8\x30\x92\xE8\x7E\xE4\x73\xFC\x1A\x5C\xDA\xFA".iter()), 0x14);
    }
    ...
    fn mad_not_present() {
        ...
        let mut tag = VALID_SECTOR_0.replace_index(57, 0b0100_0001).mock_1k();
    ...
    fn multi_application_bit_read() {
        ...
        let mut tag = VALID_SECTOR_0.replace_index(57, 0b1100_0001).mock_1k();
        ...
        let mut tag = VALID_SECTOR_0.replace_index(57, 0b1000_0001).mock_1k();
    ...
    fn invalid_mad_version() {
        ...
        // Check version 0.
        let mut tag = VALID_SECTOR_0.replace_index(57, 0b1100_0000).mock_1k();
        ...
        // Check version 3
        let mut tag = VALID_SECTOR_0.replace_index(57, 0b1100_0011).mock_1k();
    ...
    fn create_invalid_mad_cps() {
        for i in 16u8..=39 {
            ...
            if let Err(MadError::InvalidCardPublisherSectorForMadV1(s)) = result {
                assert_eq!(sector, s);
            } else {
                panic!("Expected MADv1 CPS error");
            }
        }
    }
    ...
    fn create_mad_v1_with_invalid_apps() {
        for i in 17..=39 {
            ...
            if let Err(MadError::InvalidApplicationSectorForMadV1(s)) = mad {
                assert_eq!(sector, s);
            } else {
                panic!("Expected MADv1 app sector error");
            }
        }
    }
    ...
    fn mad_v1_write() {
        ...
        let mut expected_sector = *VALID_SECTOR_0;
    ...
    fn mad_v2_write() {
        ...
        let mut expected_sector_0 = VALID_SECTOR_0_MORE_APPS.to_mad_v2();
        ...
        let mut expected_sector_16 = *VALID_SECTOR_16;
